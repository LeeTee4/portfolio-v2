...
export async function GET(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const { data, error } = await supabase.from("certificate").select("*").eq("id", params.id).single()
...
export async function PUT(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    return NextResponse.json(createApiResponse(true, data, undefined, "Certificate updated successfully"))
  } catch (error) {
    return NextResponse.json(handleApiError(error), { status: 500 })
  }
}
...
export async function DELETE(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const { error } = await supabase.from("certificate").delete().eq("id", params.id)

    if (error) {
      return NextResponse.json(createApiResponse(false, null, error.message), { status: 400 })
    }

    return NextResponse.json(createApiResponse(true, null, undefined, "Certificate deleted successfully"))
  } catch (error) {
    return NextResponse.json(handleApiError(error), { status: 500 })
  }
}
