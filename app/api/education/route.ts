...
export async function GET(request: NextRequest) {
  try {
    ...
    let query = supabase.from("education").select("*").order("start_date", { ascending: false })
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data, error } = await supabase.from("education").insert([educationData]).select().single()

    if (error) {
      return NextResponse.json(createApiResponse(false, null, error.message), { status: 400 })
    }

    return NextResponse.json(createApiResponse(true, data, undefined, "Education entry created successfully"), {
      status: 201,
    })
  } catch (error) {
    return NextResponse.json(handleApiError(error), { status: 500 })
